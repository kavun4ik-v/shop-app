...
async function loadProducts(shopId) {
    const res = await fetch(`http://localhost:3001/products?shopId=${shopId}`);
    return await res.json();
}
...
function addToCart(productId, quantity) {
    ...
    if (!product)
        return; // can't add unknown product
...
async function init() {
    ...
    shopsDiv.innerHTML = shops.map((shop) => `
...
productsDiv.addEventListener('click', (e) => {
    ...
    if (target.tagName === 'BUTTON') {
        ...
        const input = productDiv.querySelector('.quantity');
